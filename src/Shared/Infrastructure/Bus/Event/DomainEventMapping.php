...
use TechyCode\Shared\Domain\Bus\Event\DomainEventSubscriber;
...
final class DomainEventMapping
{
    ...
    {
        $this->mapping = reduce($this->eventExtractor(), $mapping, []);
    }

    public function for(string $name)
    {
        if (!isset($this->mapping[$name])) {
            throw new RuntimeException("The Domain Event Class for <$name> doesn't exist or have no subscribers");
        }
    ...
    private function eventExtractor(): callable
    {
